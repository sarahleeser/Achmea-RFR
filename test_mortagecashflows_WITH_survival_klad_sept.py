...
def calculate_cashflows_with_survival_and_principal(mortgages_df, spotdate):
    cashflows = []
    num_steps = 360
    ...
    for idx, row in mortgages_df.iterrows():
        ...
        if aflosvorm == 'annuity':
            ...
            interest, principal, balance = investment_cashflows(pv, row['Rente Nominaal'] / 12, n_periods_rvp, num_mat_months)
        else:
        ...
        # Toepassen van survival matrix op balance, BEHALVE voor 'savings' en 'annuity'
        hypotheek_id = f'Hypotheek {idx+1}'  # Survival matrices zijn opgeslagen als 'Hypotheek X'
        if aflosvorm not in ['savings', 'annuity'] and hypotheek_id in survival_matrices:
            survival_matrix = survival_matrices[hypotheek_id]
            balance = balance[:len(survival_matrix)] * survival_matrix

            # Herberekening van principal na schaling van balance
            new_principal = np.zeros_like(principal)
            new_principal[0] = pv - balance[0]  # Eerste kasstroom is de uitstaande hoofdsom - balance op tijdstip 1
            for t in range(1, len(balance)):
                new_principal[t] = balance[t - 1] - balance[t]  # Principal op elk tijdstip n is balance[n-1] - balance[n]
            
            # Zorg dat het laatste bedrag correct wordt afgerond zodat de balans 0 is
            if balance[-1] > 0:
                new_principal[-1] += balance[-1]
                balance[-1] = 0  # Zet de laatste balance op 0

            # Vervang de oorspronkelijke principal door de nieuwe berekening
...
# Voer de functie uit met de survival matrices
cashflows_with_survival_and_principal = calculate_cashflows_with_survival_and_principal(mortgages_df, spotdate)

# Print de cashflows voor alle hypotheken
for idx, cashflow in enumerate(cashflows_with_survival_and_principal):
