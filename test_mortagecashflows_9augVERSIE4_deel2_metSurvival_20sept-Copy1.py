...
# 1. Probeer de survival matrices in te laden als ze al zijn opgeslagen
try:
    with open('survival_matrices.pkl', 'rb') as f:
        survival_matrices = pickle.load(f)
    print("Survival matrices succesvol ingeladen.")
except FileNotFoundError:
    print("Survival matrices niet gevonden, aanmaken...")
...
# Pas de savings_cashflows functie aan voor het extra argument
...
# Voer de functie uit met het juiste aantal parameters
interest, principal, balance = savings_cashflows(pv, rate, n_periods_rvp, num_mat_months, months_before_spotdate)
...
def calculate_cashflows_with_survival(mortgages_df, spotdate, survival_matrices):
    ...
    for idx, row in mortgages_df.iterrows():
       ...
       # Zorg ervoor dat 'pv' de juiste waarde krijgt (Hoofdsom Restant)
        pv = row['Hoofdsom Restant']  # Dit haalt de waarde van Hoofdsom Restant op
 

        # Bereken de resterende maanden (n_remaining_periods) alleen voor 'savings' en 'Levensverzekering'
        if aflosvorm == 'savings' or aflosvorm == 'Levensverzekering':
            n_remaining_periods = num_mat_months - months_before_spotdate  # Speciale berekening voor savings
        else:
            n_remaining_periods = None  # Bij andere aflosvormen gebruiken we dit niet


        # Haal de survival matrix op voor de huidige hypotheek
        survival_matrix = survival_matrices.get(f'Hypotheek {idx}', np.ones(num_steps))

        # Bereken de originele cashflows op basis van de aflosvorm
        if aflosvorm == 'annuity':
        ...
        elif aflosvorm in ['bullet', 'Aflossingsvrij']:
            ...
            # Gebruik de speciale `n_remaining_periods` bij savings hypotheken
            interest, principal, balance = savings_cashflows(
                row['Initial Principal'], row['Rente Nominaal']/ 12 , n_periods_rvp, num_mat_months, months_before_spotdate
            )
        ...
        # Stap 1: Schaal de oorspronkelijke balance met de survival-matrix
        scaled_balance = balance * survival_matrix[:len(balance)]

        # Stap 2: Bereken de nieuwe principal op basis van de geschaalde balance
        # Principal 1: Initieel uitstaande bedrag - eerste geschaalde balance
        new_principal = [row['Initial Principal'] - scaled_balance[0]]

        # Vervolg principal: Verschillen tussen opeenvolgende geschaalde balances
        new_principal.extend([scaled_balance[i - 1] - scaled_balance[i] for i in range(1, len(scaled_balance))])

        # Laatste principal: Het resterende uitstaande bedrag volledig aflossen
        if len(scaled_balance) > 1:
            new_principal[-1] = scaled_balance[-2]

        # Stap 3: Bereken de nieuwe interest op basis van de geschaalde balance
        new_interest = scaled_balance * (row['Rente Nominaal'] / 12)

        # Vul de resterende periodes met nullen om ervoor te zorgen dat alle arrays 360 lang zijn
        new_interest = np.pad(new_interest, (0, num_steps - len(new_interest)), 'constant')
        new_principal = np.pad(new_principal, (0, num_steps - len(new_principal)), 'constant')
        scaled_balance = np.pad(scaled_balance, (0, num_steps - len(scaled_balance)), 'constant')
...
# Cashflows berekenen met survival matrices
